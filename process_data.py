...
function drawLast6Generic(c,vn,slData){{
  ...
  var thead='<thead><tr style="background:#080f18">'+
    '<th style="text-align:left;padding:6px 10px;color:#405060;font-size:.65rem;min-width:180px;border-bottom:2px solid #2d4060;white-space:nowrap">PERIOD / YEAR</th>'+
  ...
  rows6rev.forEach(function(r,i){{
    ...
    VARS.forEach(function(v){{
      var val=r[v];
      var col=isCurr?(v===vn?'#f6e05e':'#68d391'):'#8090a0';
      tbody+='<td style="padding:5px 10px;text-align:right;color:'+col+';font-weight:'+(isCurr?'700':'400')+
        ';border-bottom:1px solid #1a2535;white-space:nowrap">'+(val!=null?val.toFixed(1)+'%':'—')+'</td>';
    }});
    tbody+='</tr>';
  }});

  // thick divider
  tbody+='<tr><td colspan="'+(2+VARS.length)+'" style="height:3px;background:#1e3050;padding:0"></td></tr>';

  // Last year same week
  if(lyComp){{
    tbody+='<tr style="background:#131008">';
    tbody+='<td style="padding:5px 10px;color:#f6e05e;font-weight:600;border-bottom:1px solid #1a2535;white-space:nowrap">⬅ '+lyComp.year+' same week</td>';
    tbody+='<td style="padding:5px 10px;text-align:right;color:#405060;font-size:.64rem;border-bottom:1px solid #1a2535;white-space:nowrap">Wk '+lyComp.iso_week+'</td>';
    VARS.forEach(function(v){{tbody+=fmtCell(lyComp[v],cwVals[v],'#f6e05e');}});
    tbody+='</tr>';
  }}else{{
    tbody+='<tr style="background:#131008"><td colspan="'+(2+VARS.length)+
      '" style="padding:5px 10px;color:#405060;font-size:.65rem">Last year data not available</td></tr>';
  }}
  ...
  for(var ai=0;ai<5;ai++){{
    var comp=analogComps[ai]||null;
    var col=ACOLS[ai%5];
    var bg2=ai%2===0?'#0d1117':'#0b0f15';
    tbody+='<tr style="background:'+bg2+'">';
    if(comp){{
      tbody+='<td style="padding:5px 10px;color:'+col+';font-weight:600;border-bottom:1px solid #1a2535;white-space:nowrap">≈ '+
        comp.year+' analog'+(comp.rmse!=null?' (RMSE='+comp.rmse+')':'')+'</td>';
      tbody+='<td style="padding:5px 10px;text-align:right;color:#405060;font-size:.64rem;border-bottom:1px solid #1a2535;white-space:nowrap">Wk '+comp.iso_week+'</td>';
      VARS.forEach(function(v){{tbody+=fmtCell(comp[v],cwVals[v],col);}});
    }}else{{
      tbody+='<td colspan="'+(2+VARS.length)+'" style="padding:5px 10px;color:#405060;font-size:.65rem;border-bottom:1px solid #1a2535">Analog '+(ai+1)+' not available</td>';
    }}
    tbody+='</tr>';
  }}
  ...
  c.innerHTML='<div style="font-size:.63rem;color:#607080;margin-bottom:5px">'
    +'★ = current week &nbsp;·&nbsp; diff vs current week in brackets &nbsp;·&nbsp; values = % cotton area in drought</div>'
    +'<div style="overflow-x:auto;border:1px solid #1e2a3a;border-radius:6px">'
    +'<table style="border-collapse:collapse;font-size:.73rem;width:100%;min-width:520px">'
