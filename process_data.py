...
function drawLast6Generic(c,vn,slData){{
  if(!slData||!slData.variables){{c.innerHTML='<p style="color:#607080;padding:10px">No data.</p>';return;}}
  var vd=slData.variables[vn];
  if(!vd)return;
  ...
  var lyComp=null;
  var anaComps=[];
  comps.forEach(function(x){{if(x.is_last_yr){{lyComp=x;}}else{{anaComps.push(x);}}}});
  var cwRow=rows6.length?rows6[rows6.length-1]:null;
  var cwVals={{}};
  VARS.forEach(function(v){{cwVals[v]=cwRow?cwRow[v]:null;}});

  function valCell(val,color,bold){{
    var st='padding:5px 12px;text-align:right;border-bottom:1px solid #1e2a3a;white-space:nowrap;color:'+color+';'+(bold?'font-weight:700;':'');
    return '<td style="'+st+'">'+(val!=null?val.toFixed(1)+'%':'&mdash;')+'</td>';
  }}
  function diffCell(val,refVal,color){{
    if(val==null)return '<td style="padding:5px 12px;text-align:right;border-bottom:1px solid #1e2a3a;color:#304050">&mdash;</td>';
    var d=(refVal!=null)?(val-refVal):null;
    var ds=d!=null?' <span style="font-size:.6rem;color:'+(d>0.05?'#fc8181':d<-0.05?'#68d391':'#607080')+'">'+(d>=0?'+':'')+d.toFixed(1)+'</span>':'';
    return '<td style="padding:5px 12px;text-align:right;border-bottom:1px solid #1e2a3a;white-space:nowrap;font-weight:600;color:'+color+'">'+val.toFixed(1)+'%'+ds+'</td>';
  }}

  var p=[];
  p.push('<table style="border-collapse:collapse;font-size:.73rem;width:100%;min-width:500px">');
  p.push('<thead><tr style="background:#080f18">');
  p.push('<th style="text-align:left;padding:6px 12px;color:#405060;font-size:.65rem;min-width:220px;border-bottom:2px solid #2d4060">PERIOD / YEAR</th>');
  p.push('<th style="padding:6px 12px;text-align:right;color:#405060;font-size:.65rem;border-bottom:2px solid #2d4060">Date</th>');
  VARS.forEach(function(v){{
    p.push('<th style="padding:6px 12px;text-align:right;border-bottom:2px solid #2d4060;color:'+VC[v]+';font-size:.7rem;font-weight:'+(v===vn?700:500)+'">'+v+'</th>');
  }});
  p.push('</tr></thead><tbody>');

  var rev=rows6.slice().reverse();
  rev.forEach(function(r,i){{
    var isCurr=(r.iso_week===latestIW);
    var bg=isCurr?'#071a0e':(i%2===0?'#0d1117':'#0b0f17');
    var nc=isCurr?'#68d391':'#7090a0';
    p.push('<tr style="background:'+bg+'">');
    p.push('<td style="padding:5px 12px;color:'+nc+';font-weight:'+(isCurr?700:400)+';border-bottom:1px solid #1e2a3a;white-space:nowrap">'+(isCurr?'&#9733; ':'')+currYr+' &mdash; '+(r.label||('Wk'+r.iso_week))+'</td>');
    p.push('<td style="padding:5px 12px;text-align:right;color:#405060;font-size:.64rem;border-bottom:1px solid #1e2a3a">'+(r.date||'')+'</td>');
    VARS.forEach(function(v){{p.push(valCell(r[v],isCurr?(v===vn?'#f6e05e':'#68d391'):'#8090a0',isCurr));}});
    p.push('</tr>');
  }});

  var nc2=2+VARS.length;
  p.push('<tr><td colspan="'+nc2+'" style="height:3px;background:#1e3050;padding:0"></td></tr>');

  if(lyComp){{
    p.push('<tr style="background:#131008">');
    p.push('<td style="padding:5px 12px;color:#f6e05e;font-weight:600;border-bottom:1px solid #1e2a3a;white-space:nowrap">&#8592; '+lyComp.year+' same week</td>');
    p.push('<td style="padding:5px 12px;text-align:right;color:#405060;font-size:.64rem;border-bottom:1px solid #1e2a3a">Wk '+lyComp.iso_week+'</td>');
    VARS.forEach(function(v){{p.push(diffCell(lyComp[v],cwVals[v],'#f6e05e'));}});
    p.push('</tr>');
  }}else{{
    p.push('<tr style="background:#131008"><td colspan="'+nc2+'" style="padding:5px 12px;color:#405060;border-bottom:1px solid #1e2a3a">Last year not available</td></tr>');
  }}

  p.push('<tr><td colspan="'+nc2+'" style="height:3px;background:#1e3050;padding:0"></td></tr>');

  for(var ai=0;ai<5;ai++){{
    var comp=anaComps[ai]||null;
    var acol=ACOLS[ai%5];
    p.push('<tr style="background:'+(ai%2===0?'#0d1117':'#0b0f17')+'">');
    if(comp){{
      p.push('<td style="padding:5px 12px;color:'+acol+';font-weight:600;border-bottom:1px solid #1e2a3a;white-space:nowrap">&#8776; '+comp.year+' analog'+(comp.rmse!=null?' (RMSE='+comp.rmse+')':'')+'</td>');
      p.push('<td style="padding:5px 12px;text-align:right;color:#405060;font-size:.64rem;border-bottom:1px solid #1e2a3a">Wk '+comp.iso_week+'</td>');
      VARS.forEach(function(v){{p.push(diffCell(comp[v],cwVals[v],acol));}});
    }}else{{
      p.push('<td colspan="'+nc2+'" style="padding:5px 12px;color:#405060;border-bottom:1px solid #1e2a3a">Analog '+(ai+1)+' not available</td>');
    }}
    p.push('</tr>');
  }}

  p.push('</tbody></table>');
  c.innerHTML='<div style="font-size:.62rem;color:#607080;margin-bottom:5px">&#9733; = current week &nbsp;&middot;&nbsp; diff vs current in brackets &nbsp;&middot;&nbsp; values = % cotton area in drought</div>'
    +'<div style="overflow-x:auto;border:1px solid #1e2a3a;border-radius:6px">'+p.join('')+'</div>';
}}
...
function drawOneLineGeneric(cid,vn,slData,analogsData){{
  ...
  // Last 5 years — bright distinct palette, shown before analogs
  var LAST5_COLS=["#ff9f43","#54a0ff","#ff6b9d","#00d2d3","#ffd32a"];
  last5.forEach(function(yr,i){{
    var col=shownInBar[String(yr)]?null:LAST5_COLS[i%5];
    if(col)addBadge(yr,col,"");
  }});
  // Analogs — ACOLS bright colors
  analogs.forEach(function(yr,i){{
    var col=shownInBar[String(yr)]?ACOLS[i%5]:ACOLS[i%5];
    addBadge(yr,col," ≈");
  }});
  ...
  function drawChart(){{
    ...
    // ── Last 5 years lines — bright distinct colors ──
    var LAST5_COLS=["#ff9f43","#54a0ff","#ff6b9d","#00d2d3","#ffd32a"];
    last5.forEach(function(yr,i){{
      if(analogs.indexOf(yr)>=0)return;
      mkLine2(yr,LAST5_COLS[i%5],1.5,null,true);
    }});
